...
int main(void)
{
	...
	////////////// Exercise 1 Step 2.9
	err = myqueue.enqueueWriteBuffer(Buffer_In,false,0,sizeof(cl_float)*vectorSize,X,NULL,NULL);
	checkErr(err, "WriteBuffer");
	err = myqueue.enqueueWriteBuffer(Buffer_In,false,0,sizeof(cl_float)*vectorSize,Y,NULL,NULL);
	...
	std::ifstream aocx_stream("../SimpleKernel.cl", std::ios::in|std::ios::binary);
